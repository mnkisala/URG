...
#include "URGHud.h"
#include "URGWheelFront.h"
#include "URGWheelRear.h"
...
const FName AURGPawn::LookUpBinding("LookUp");
const FName AURGPawn::LookRightBinding("LookRight");
const FName AURGPawn::EngineAudioRPM("RPM");
...
AURGPawn::AURGPawn() {
  ...
  {
    VehicleMovement->WheelSetups[0].WheelClass = UURGWheelFront::StaticClass();
    ...
    VehicleMovement->WheelSetups[1].WheelClass = UURGWheelFront::StaticClass();
    ...
    VehicleMovement->WheelSetups[2].WheelClass = UURGWheelRear::StaticClass();
    ...
    VehicleMovement->WheelSetups[3].WheelClass = UURGWheelRear::StaticClass();
  ...
  VehicleMovement->EngineSetup.TorqueCurve.GetRichCurve()->AddKey(1890.0f,
                                                                  500.0f);
  VehicleMovement->EngineSetup.TorqueCurve.GetRichCurve()->AddKey(5730.0f,
                                                                  400.0f);

  // This works because the AxleType has been setup on the wheels
  VehicleMovement->DifferentialSetup.DifferentialType =
      EVehicleDifferential::RearWheelDrive;

  // Adjust the steering
  VehicleMovement->SteeringSetup.SteeringCurve.GetRichCurve()->Reset();
  VehicleMovement->SteeringSetup.SteeringCurve.GetRichCurve()->AddKey(0.0f,
                                                                      1.0f);
  VehicleMovement->SteeringSetup.SteeringCurve.GetRichCurve()->AddKey(40.0f,
                                                                      0.7f);
  VehicleMovement->SteeringSetup.SteeringCurve.GetRichCurve()->AddKey(120.0f,
                                                                      0.6f);
  ...
  UPrimitiveComponent *UpdatedPrimitive =
      Cast<UPrimitiveComponent>(VehicleMovement->UpdatedComponent);
  if (UpdatedPrimitive) {
    UpdatedPrimitive->BodyInstance.COMNudge = FVector(8.0f, 0.0f, -15.0f);
  }

  // Set the inertia scale. This controls how the mass of the vehicle is
  // distributed.
  ...
  // SoundCue(TEXT("/Game/Vehicles/Sound/Engine_Loop_Cue.Engine_Loop_Cue"));
  // EngineSoundComponent =
  // CreateDefaultSubobject<UAudioComponent>(TEXT("EngineSound"));
...
void AURGPawn::SetupPlayerInputComponent(
...
void AURGPawn::MoveRight(float Val) {
  GetVehicleMovementComponent()->SetSteeringInput(Val);
}

void AURGPawn::OnHandbrakePressed() {
  GetVehicleMovementComponent()->SetHandbrakeInput(true);
}

void AURGPawn::OnHandbrakeReleased() {
  GetVehicleMovementComponent()->SetHandbrakeInput(false);
}

void AURGPawn::OnToggleCamera() { EnableIncarView(!bInCarCameraActive); }

void AURGPawn::EnableIncarView(const bool bState) {
...
void AURGPawn::Tick(float Delta) {
  Super::Tick(Delta);
  ...
  UChaosWheeledVehicleMovementComponent *WheeledVehicle =
      static_cast<UChaosWheeledVehicleMovementComponent *>(
          GetVehicleMovement());
  float RPMToAudioScale = 2500.0f / WheeledVehicle->GetEngineMaxRotationSpeed();
  // EngineSoundComponent->SetFloatParameter(EngineAudioRPM,
  // WheeledVehicle->GetEngineRotationSpeed()*RPMToAudioScale);
}

void AURGPawn::BeginPlay() {
  Super::BeginPlay();
...
void AURGPawn::UpdateHUDStrings() {
...
void AURGPawn::UpdatePhysicsMaterial() {
